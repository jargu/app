...
const Mailbox = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Mensajes'));
    });
    ...
    const setArchive = () => {
        if (ids.length) {
            ...
            if (selectedTab === 'archive') {
                showMessage(ids.length + ' El mensaje ha sido movido de Archivado.');
            } else {
                showMessage(ids.length + ' El mensaje ha sido agregado en Archivado.');
            }
            searchMails(false);
        }
    };
    ...
    const setSpam = () => {
        if (ids.length) {
            ...
            if (selectedTab === 'spam') {
                showMessage(ids.length + ' El mensaje ha sido movido de Spam.');
            } else {
                showMessage(ids.length + ' El mensaje ha sido agregado a Spam.');
            }
            searchMails(false);
        }
    };
    ...
    const setGroup = (group: any) => {
        if (ids.length) {
            ...
            showMessage(ids.length + ' El mensaje ha sido agrupado en ' + group.toUpperCase());
            clearSelection();
            setTimeout(() => {
                searchMails(false);
            });
        ...
        if (ids.length) {
            ...
            for (let item of items) {
                if (type === 'trash') {
                    ...
                    showMessage(totalSelected + ' El mensaje ha sido Eliminado.');
                    searchMails(false);
                } else if (type === 'read') {
                    item.isUnread = false;
                    showMessage(totalSelected + ' El mensaje ha sido marcado como leido.');
                } else if (type === 'unread') {
                    item.isUnread = true;
                    showMessage(totalSelected + ' El mensaje ha sido marcado como sin leer.');
                } else if (type === 'important') {
                    item.isImportant = true;
                    showMessage(totalSelected + ' El mensaje ha sido marcado como importante.');
                } else if (type === 'unimportant') {
                    item.isImportant = false;
                    showMessage(totalSelected + ' El mensaje ha sido marcado como no importante.');
                } else if (type === 'star') {
                    item.isStar = true;
                    showMessage(totalSelected + ' El mensaje ha sido marcado como favorito.');
                }
                //restore & permanent delete
                else if (type === 'restore') {
                    item.type = 'inbox';
                    showMessage(totalSelected + ' El mensaje ha sido restaurado.');
                    searchMails(false);
                } else if (type === 'delete') {
                    setMailList(mailList.filter((d: any) => d.id !== item.id));
                    showMessage(totalSelected + ' Mensaje eliminado permanentemente.');
                    searchMails(false);
                }
            }
            clearSelection();
    ...
    const saveMail = (type: any, id: any) => {
        if (!params.to) {
            showMessage('La dirección de correo es requerido.', 'error');
            return false;
        }
        if (!params.title) {
            showMessage('El asunto del correo es requerido.', 'error');
            return false;
        }
        ...
        if (type === 'save' || type === 'save_reply' || type === 'save_forward') {
            ...
            searchMails();
            showMessage('El mensaje ha sido guardado exitosamenet en draft.');
        } else if (type === 'send' || type === 'reply' || type === 'forward') {
            //saved to sent mail
            obj.type = 'sent_mail';
            mailList.splice(0, 0, obj);
            searchMails();
            showMessage('El mensaje ha sido enviado exitosamenet.');
        }
            ...
            <div className="flex gap-5 relative sm:h-[calc(100vh_-_150px)] h-full">
                ...
                >
                    <div className="flex flex-col h-full pb-16">
                        <div className="pb-5">
                            <button className="btn btn-success w-full" type="button" onClick={() => openMail('add', null)}>
                                Nuevo mensaje
                            </button>
                        </div>
                            ...
                            <div className="space-y-1">
                                ...
                                >
                                    <div className="flex items-center">
                                        ...
                                        <div className="ltr:ml-3 rtl:mr-3">Enviados</div>
                                    </div>
                                </button>
                                ...
                                >
                                    <div className="flex items-center">
                                        ...
                                        <div className="ltr:ml-3 rtl:mr-3">Basurero</div>
                                    </div>
                                </button>
                                    ...
                                    {({ open }) => (
                                        <>
                                            <Disclosure.Button className="w-full flex items-center p-2 hover:bg-white-dark/10 rounded-md dark:hover:text-primary hover:text-primary dark:hover:bg-[#181F32] font-medium h-10">
                                                ...
                                                <div className="ltr:ml-3 rtl:mr-3">{open ? 'Menos' : 'Más'}</div>
                                            </Disclosure.Button>
                                            ...
                                            <Disclosure.Panel as="ul" unmount={false} className="mt-1 space-y-1">
                                                <li>
                                                    ...
                                                    >
                                                        ...
                                                        <div className="ltr:ml-3 rtl:mr-3">Archivados</div>
                                                    </button>
                                                </li>
                                                <li>
                                                    <button
                                                    ...
                                                    >
                                                        ...
                                                        <div className="ltr:ml-3 rtl:mr-3">Importantes</div>
                                                    </button>
                                                </li>
                                            </Disclosure.Panel>
                                        </>
                                ...
                                >
                                    <div className="flex items-center">
                                        ...
                                        <div className="ltr:ml-3 rtl:mr-3">Nueva reunión</div>
                                    </div>
                                </button>
                                <button
                                    type="button"
                                ...
                                >
                                    <div className="flex items-center">
                                        ...
                                        <div className="ltr:ml-3 rtl:mr-3">Unirte a reunión</div>
                                    </div>
                                </button>
                                <div className="h-px border-b border-white-light dark:border-[#1b2e4b]"></div>
                            </div>
                    ...
                    {!selectedMail && !isEdit && (
                        <div className="flex flex-col h-full">
                                ...
                                <div className="flex items-center w-full sm:w-auto">
                                    ...
                                    {selectedTab === 'trash' && (
                                        <ul className="flex flex-1 items-center sm:flex-none gap-4 ltr:sm:mr-3 rtl:sm:ml-4">
                                            <li>
                                                <div>
                                                    <Tippy content="Eliminar permanentemente">
                            ...
                            {pagedMails.length ? (
                                ...
                                <div className="grid place-content-center min-h-[300px] font-semibold text-lg h-full">Datos no disponibles</div>
                            )}
                        </div>
                    )}
                                ...
                                <div className="flex flex-wrap">
                                    ...
                                    <div className="ltr:mr-2 rtl:ml-2 flex-1">
                                        <div className="flex items-center">
                                            ...
                                            <div className="text-white-dark whitespace-nowrap">1 dia atrás</div>
                                        </div>
                    ...
                    {isEdit && (
                        <div className="relative">
                            <div className="py-4 px-6 flex items-center">
                                ...
                                <h4 className="text-lg text-gray-600 dark:text-gray-400 font-medium">Mensaje</h4>
                            </div>
                            ...
                            <form className="p-6 grid gap-6">
                                ...
                                <div>
                                    <input id="cc" type="text" className="form-input" placeholder="Ingresa Cc" defaultValue={params.cc} onChange={(e) => changeValue(e)} />
                                </div>

                                <div>
                                    <input id="title" type="text" className="form-input" placeholder="Ingresa el Asunto" defaultValue={params.title} onChange={(e) => changeValue(e)} />
                                </div>
